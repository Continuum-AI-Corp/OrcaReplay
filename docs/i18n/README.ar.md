...
| مُتحقَّق منه على وكيل حقيقي | Claude Code وهو يصلح علّة حقيقية فعلًا: سُجّل، وأُعيد دون شبكة من أوله إلى آخره، وفُرّع من نقطة تحقق، وصُدّر. وجدنا في الطريق أربع علل، وأُصلحت كلها — انظر أدناه |
...
### ما الذي وجده وكيل حقيقي

كل ما سبق بُني في مواجهة تجهيزات اختبار. وأول تشغيل حقيقي لـ Claude Code سُجّل عبره كسر أربعة أشياء
دفعةً واحدة، وكل واحد منها من نوع لا تستطيع أي تجهيزة أن تنتجه:

- **انزياح من ستة عشر حرفًا قُيّم بـ 217,568.** كانت المسافة تُقاس بالبادئة واللاحقة المشتركتين لجسم
  الطلب كله، وClaude Code يحمل معرّف جلسة في موجّه نظامه *ومعرّفًا آخر* في وصف إحدى الأدوات — فصار كل
  ما بينهما من 200 كيلوبايت محسوبًا كتغيّر، ولم يستطع أي طلب بلوغ الدرجة الثانية. صارت المسافة الآن
  تُجمع لكل حقل على حدة، وداخل الحقل سطرًا سطرًا.
- **جعل التنقيحُ التطابقَ الدقيق بعيد المنال.** بصمات العناصر النائبة مملّحة لكل تشغيل عن قصد، فما كان
  لطلب مسجَّل أن يساوي نفسه مرة أخرى. صار المطابِق ينقّح الطلب الوارد بالسياسة نفسها ويقارن *نوع*
  السرّ لا بصمته — ويبلّغ عن هذا الطيّ، لأنه تقريب.
- **وكان التنقيح يكسر كل تفريع أيضًا.** معرّفات `tool_use` وتواقيع كتل التفكير سلاسل عالية العشوائية،
  فكان المسح يستبدلها؛ والتفريع يعيد تشغيل تلك الأدوار، فيعيدها الوكيل كما هي، فتردّ الواجهة بـ
  `400`. أما قيم البروتوكول التي يجب أن تعود كما ذهبت فصارت مستثناة من هذا *التخمين* — لا من قواعد
  بيانات الاعتماد أبدًا.
- **الإعادة تشغّل الأدوات فعلًا من جديد.** لا يعترض orca تنفيذ الأدوات، فالوكيل يشغّل `npm test` مرة
  أخرى حقًّا، وهي تطبع مددها من جديد حقًّا. والطلب الذي ينحصر اختلافه داخل مخرجات أداة صار يُخدم من
  التسجيل ويُسجَّل تباعدًا من مستوى `major` بدل أن يوقف الإعادة.

ذلك التشغيل يُعاد الآن دون شبكة من أوله إلى آخره — `reused=7/7 exact=2 divergences=5 unmatched=0
exit=0`، وكل تقريب فيه مسمّى — وتفريعٌ منه يصل إلى الشجرة نفسها التي وصل إليها التسجيل. وإن كان لديك
تسجيل ما يزال يخطئ فيه، فذلك أنفع شيء يمكنك إرساله.
