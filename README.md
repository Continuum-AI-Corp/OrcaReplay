...
**What "egress blocked" means, exactly.** On replay the proxy refuses to forward anything it cannot serve from the trace, so no model is called and no tokens are spent. That is the whole of the guarantee. A replay still *executes the recorded tool calls for real*, and a tool that opens its own socket — a shell command running `curl`, an MCP server fetching something — never passes through the proxy and reaches the network as usual. Replay is not a sandbox; if you need one, run it inside one.
