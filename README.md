...
| **TypeSafe / Jev** | `orca record generic-openai -- python your_app.py` — `TYPESAFE_BASE_URL`, which `generic-openai` now forwards | works — a System One call records and replays offline at `retrieval=1/1`, [in CI](test/integrations/). Not a model *exchange*: typed propositions in, probabilities out, so there is no conversation to fork |
