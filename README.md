...
| **Vercel AI SDK** | `OPENAI_BASE_URL` via `@ai-sdk/openai` (since 3.0.72) — `orca record generic-openai --`; fetch hook (`orca record node --`) when the origin is compiled in | works — env-aware SDK clients via `generic-openai`; compiled-in origins via the node preload, [in CI](test/integrations/) |
